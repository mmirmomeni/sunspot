...
/* 
 This file is an archive of the prediction horizon related code; it's not broken,
 but should be refactored when we need it.
 */
...
//    //! Calculate fitness of ind.
//	template <typename Individual, typename RNG, typename EA>
//	void test(const std::string& filename, Individual& ind, RNG& rng, EA& ea) {
//        namespace bnu=boost::numeric::ublas;
//        mkv::markov_network net = mkv::make_markov_network(_desc, ind.repr().begin(), ind.repr().end(), rng, ea);
//        datafile df(filename);
//        df.add_field("observed").add_field("predicted");
//
//        // vector of outputs from the MKV network, initialized to the same size
//        // as R:
//        vector_type output(_observed.size1());
...
//            // MKV networks:
//            output(i) = static_cast<double>(algorithm::range_pair2int(net.begin_output(), net.end_output()));
//            df.write(_IntegerObserved(i)).write(output(i)).endl();
//        }
//    }
//};
//
...
//
//        ea.fitness_function().test(get<ANALYSIS_OUTPUT>(ea,"test_sunspot"), ind, ea.rng(), ea);
//    }
//};
//
//#endif
